...
if (!function_exists('adforestAPI_post_ad_get')) {
    ...
    function adforestAPI_post_ad_get($request) {
        ...
        if ($is_update != "") {
            $data['title'] = __("Edit Add", "adforest-rest-api");
        } else {
            $data['title'] = __("Add Post", "adforest-rest-api");
        }
        ...
        if ($is_update != "") {
            ...
            if (get_post_status($pid) == 'rejected') {
        ...
        $data['profile']['location'] = adforestAPI_getPostAdFields('glocation_textfield', 'ad_location', '', 0, __("Address", "adforest-rest-api"), '', $sb_location, '3', true, $sb_location);
...
if (!function_exists('adforestAPI_delete_ad_image')) {
    ...
    function adforestAPI_delete_ad_image($request) {
        ...
        $images = array();
        if ($ad_id != "") {
            $images = adforestAPI_get_ad_image($ad_id, $maxLimit, 'thumb', false);
        }
        $remaningImages = $maxLimit - count($images);
...
if ($my_theme->get('Name') != 'adforest' && $my_theme->get('Name') != 'adforest child') {
    add_action('woocommerce_thankyou', 'adforestAPI_custom_woocommerce_auto_complete_order');
}
