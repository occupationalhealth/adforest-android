 ...
 */
/* Ads Loop Starts */
add_action('plugins_loaded', 'adforestAPI_add_new_image_size');
...
if (!function_exists('adforestApi_featuredAds_slider')) {
    ...
    function adforestApi_featuredAds_slider($userid = '', $status = '', $adType = '', $limit = - 1, $term_id = '', $adStatus = 'publish', $other = '') {
        ...
        $category = '';
        if ($term_id != "") {
            $category = array(array('taxonomy' => 'ad_cats', 'field' => 'term_id', 'terms' => $term_id,),);
        }
...
if (!function_exists('adforestAPI_getSubCats')) {
    ...
    function adforestAPI_getSubCats($field_type = '', $field_type_name = '', $term_type = 'ad_cats', $only_parent = 0, $name = '', $mainTitle = '', $show_count = true) {
...
add_action('rest_api_init', 'adforestAPI_some_test', 0);

function adforestAPI_some_test() {
    register_rest_route('adforest/v1', '/sometest/', array('methods' => WP_REST_Server::READABLE, 'callback' => 'adforestAPI_adPrice_types',));
}
...
if (!function_exists('adforestAPI_getPostAdFields')) {
    ...
    function adforestAPI_getPostAdFields($field_type = '', $field_type_name = '', $term_type = 'ad_cats', $only_parent = 0, $name = '', $mainTitle = '', $defaultValue = '', $has_page_number = 1, $is_required = false, $update_val = '', $ad_id = '') {
        ...
        if ($field_type == "select") {
            ...
            if ('ad_price_type' == $field_type_name) {
                $termsArr[] = array(
                    'id' => '',
                    'name' => "Select Option",
                    'has_sub' => false,
...
if (!function_exists('adforestAPI_do_register')) {
    ...
    function adforestAPI_do_register($email = '', $password = '') {
        ...
        $sb_allow_ads = Redux::getOption('adforestAPI', 'sb_allow_ads');
        if (isset($sb_allow_ads) && $sb_allow_ads == true) {
            $free_ads = Redux::getOption('adforestAPI', 'sb_free_ads_limit');
            $featured_ads = Redux::getOption('adforestAPI', 'sb_featured_ads_limit');
            $package_validity = Redux::getOption('adforestAPI', 'sb_package_validity');
            $allow_featured_ads = Redux::getOption('adforestAPI', 'sb_allow_featured_ads');
...
if (!function_exists('adforestAPI_payment_types')) {
    ...
    function adforestAPI_payment_types($key = '', $type = '') {
        ...
        if ($type != "ios") {
            ...
            $arr['paypal'] = __('PayPal', 'adforest-rest-api');
...
if (!function_exists('adforestAPI_get_all_countries')) {
    ...
    function adforestAPI_get_all_countries() {
        ...
        foreach ($countries as $country) {
            $res[$country
                    ->post_excerpt] = $country->post_title;
        }
        return $res;
    }
